...
class NumberQuestions:
    ...
    def __init__(self):
        ...
        # set up GUI frame
        self.question_frame = Frame(padx=50, pady=60)
    ...
    # starts the quiz by checking the input
    def start_quiz(self):
        min_value = 1.0  # edit out when implementing (only for testing)
        max_value = 10.0
        result = self.check_question(min_value, max_value)
        self.output_answer()
        if result != "invalid":
            print("Start the quiz with {} questions".format(result))
