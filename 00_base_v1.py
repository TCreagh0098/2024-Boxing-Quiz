...
class ChooseWeight:
    ...
    def weight_select(self, weight_class):
        ...
        Quiz(self.quiz_window, weight_class)
...
class Quiz:
    ...
    def __init__(self, quiz_window, weight_class):
    ...
    def update_question(self):
        ...
        try:
            boxer, date = random.choice(weight_classes[weight_class])
            print(f"Selected boxer: {boxer}, Date: {date}")
        ...
        self.question_label.config(text=f"Who became undisputed {weight_class} champion in {date}?")
        ...
        while len(boxers) < 4 and attempts < 100:
            boxer_name, _ = random.choice(weight_classes[weight_class])
