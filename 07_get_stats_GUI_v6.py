...
class DisplayStats:
    def __init__(self, partner, attempts):
        ...
        for item in range(len(all_attempts_labels)):
            self.attempts_data_label = Label(self.data_frame, text=all_attempts_labels[item][0],
                                             bg=all_attempts_labels[item][1], width="20", height="2", padx=5,
                                             borderwidth=1, relief="solid",
                                             fg="#FFFFFF")
            self.attempts_data_label.grid(row=0, column=0, padx=0, pady=0)

        # Display total statistics
        stats_labels = [
            ("Total Questions Asked", total_questions_asked),
            ("Total Correct Answers", total_correct_answers),
            ("Average Correct Percentage", f"{average_correct_percentage}%")
        ]

        for label_text, value in stats_labels:
            ...
            self.label.grid(row=row, column=1, padx=0, pady=0)
            row += 1
            self.value = Label(self.data_frame, text=value, bg=head_back, width=30, height=2,
                               padx=5, borderwidth=1, relief="solid", fg=text_color)
            self.value.grid(row=0, column=1, padx=0, pady=0)
            row += 1
