...
class DisplayStats:
    def __init__(self, partner, attempts):
        ...
        self.data_frame = Frame(self.stats_frame, bg="#FFFFFF", borderwidth=1, relief="solid")
        ...
        for item in range(len(all_labels)):
            self.data_label = Label(self.data_frame, text=all_labels[item][0], bg=all_labels[item][1], width="20",
        ...
        for item in range(len(all_attempts_labels)):
            self.attempts_data_label = Label(self.recent_attempts_frame, text=all_attempts_labels[item][0],
                                             ...
                                             fg="#FFFFFF")
            self.attempts_data_label.grid(row=item // 5, column=item % 5, padx=0, pady=0)

        self.dismiss_button = Button(self.stats_frame, font=("Arial", "12", "bold"), text="Dismiss", bg="#990000",
